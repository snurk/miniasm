...
//    fprintf(stderr, "    -B          only one direction of an arc is present in input PAF\n"); // deprecated; for backward compatibility
  ...
  fprintf(stderr, "[M::%s] ===> Reading read mappings <===\n", __func__);
  ...
  if (strcmp(outfmt, "bed") == 0) {
    ...
    fprintf(stderr, "[M::%s] ===> Graph cleaning <===\n", __func__);
    ...
    if (stage >= 9) {
      ...
      for (i = 0; i < opt.n_rounds; ++i) {
        float r;
        if (opt.n_rounds == 1)
          r = opt.max_ovlp_drop_ratio;
        else
          r = opt.min_ovlp_drop_ratio + (opt.max_ovlp_drop_ratio - opt.min_ovlp_drop_ratio) / (opt.n_rounds - 1) * i;
    ...
    if (strcmp(outfmt, "ug") == 0) {
      fprintf(stderr, "[M::%s] ===> Generating unitigs <===\n", __func__);
      ug = ma_ug_gen(sg);
